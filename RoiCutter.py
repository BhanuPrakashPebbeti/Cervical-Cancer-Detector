...
class Fuse(tf.keras.layers.Layer):
    '''Fusion layer'''
    def _init_(self, epsilon=1e-4, **kwargs):
        super(Fuse, self)._init_(**kwargs)
...
def create_model(input_shape ,c,stride_obj=2,wbifpn=False):
    '''model'''
    c1,c2,c3,c4 = c
    ...
    prediction_model.load_weights('models/obj_model3.h5')
    return prediction_model

def _ctdet_decode(hm, reg, wh, k=20, output_size_obj = 256, output_stride=2):
    bboxes = []
    scores = []
...
def _nms(heat, kernel=5):
    ...
    return heat * keep
def cut_image(model,imagecv,input_size_obj=512,stride_obj = 2):
